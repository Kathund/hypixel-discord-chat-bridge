...
import { getUUID } from '../../src/contracts/API/PlayerDBAPI';
...
export const getLatestProfile = async (uuid: any, options = { museum: false }) => {
  ...
  const [{ data: playerRes }, { data: profileRes }] = await Promise.all([
    axios.get(`https://api.hypixel.net/player?key=${minecraft.API.hypixelAPIkey}&uuid=${uuid}`),
    axios.get(`https://api.hypixel.net/skyblock/profiles?key=${minecraft.API.hypixelAPIkey}&uuid=${uuid}`),
  ]).catch((error) => {
  ...
  const profileData = profileRes.profiles.find((a: any) => a.selected) || null;
  if (profileData == null) {
