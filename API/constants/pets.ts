...
class Bat extends Pet {
  get stats() {
    ...
    if (this.rarity >= MYTHIC) {
      (stats as any).sea_creature_chance = this.level * 0.05;
    }
    return stats;
  }
    ...
    const riftSouls =
      'objectives' in (this.profile ?? {})
        ? Object.entries(this.profile.objectives).find(
            ([key, value]) => key.startsWith('rift_') && key.endsWith('_soul') && (value as any).status === 'COMPLETE'
          )?.length ?? 0
        : 0;
  ...
  get second() {
    ...
    return {
      ...
      desc: [
        ...
        !maxTier
          ? `§7Next Upgrade: §a+${(next as any).defense} ${SYMBOLS.defense} §7§ §f+${(next as any).true_defense} ${
              SYMBOLS.true_defense
            } §7(§a${blazeKills.toLocaleString()}§7/§c${(next as any).kills.toLocaleString()}§7)`
          : '§aMAXED OUT!',
      ],
    };
  }
