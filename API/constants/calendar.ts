...
import { nth, parseTimestamp } from '../../src/contracts/helperFunctions';
...
export const buildSkyblockCalendar = (events: any, from: any, to: any, years: any, stopAtYearEnd = true) => {
  const now = Date.now();
  ...
  for (let i = 0; i < toToYears; i++) {
    for (const [event, { name, times: times_ }] of Object.entries(eventTimes)) {
      ...
      (eventList as any)[event].name = name;
      (eventList as any)[event].duration = duration;
      ...
      for (const { start: start_, end: end_ } of times_) {
        ...
        if (name === 'Traveling Zoo') (o as any).pet = getZooPet(fromDate + msTill);

        (eventList as any)[event].events.push(o);
      }
    }
  }

  Object.keys(eventList).forEach((key) => {
    (eventList as any)[key].events = getUniqueListBy((eventList as any)[key].events, 'start_timestamp')
      .filter(({ start_timestamp }: any) => start_timestamp < toDate)
      .sort((a, b) => (a as any).start_timestamp - (b as any).start_timestamp);
  });
  ...
  for (const event of eventsToFilter) {
    (filteredEvents as any)[event] = (eventList as any)[event];
  }
...
export const buildSkyblockEvents = () => {
  const o: any = {};
