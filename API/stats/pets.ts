...
import { pet_data, petStats, pet_items, rarityColors, PET_RARITY_OFFSET, PET_LEVELS } from '../constants/pets';
import { titleCase, capitalize, renderLore, formatNumber } from '../constants/functions';
import { pet_skins } from '../constants/skins';
...
export const getPets = (profile: any) => {
  let output: any = [];
  ...
  for (const pet of profile.pets) {
    ...
    const petData = (pet_data as any)[pet.type] || {
    ...
    const petInstance = new (petStats as any)[searchName](rarity, pet.level, pet.extra);
    pet.stats = Object.assign({}, petInstance.stats);
    pet.ref = petInstance;

    if (pet.heldItem) {
      const { heldItem } = pet;
      let heldItemObj = (pet_items as any)[heldItem];

      if (heldItem in (pet_items as any)) {
        for (const stat in (pet_items as any)[heldItem]?.stats) {
          pet.stats[stat] = (pet.stats[stat] || 0) + (pet_items as any)[heldItem].stats[stat];
        }
        for (const stat in (pet_items as any)[heldItem]?.statsPerLevel) {
          pet.stats[stat] = (pet.stats[stat] || 0) + (pet_items as any)[heldItem].statsPerLevel[stat] * pet.level;
        }
        for (const stat in (pet_items as any)[heldItem]?.multStats) {
          if (pet.stats[stat]) {
            pet.stats[stat] = (pet.stats[stat] || 0) * (pet_items as any)[heldItem].multStats[stat];
          }
        }
        if ('multAllStats' in (pet_items as any)[heldItem]) {
          for (const stat in pet.stats) {
            pet.stats[stat] *= (pet_items as any)[heldItem].multAllStats;
          }
        }
      }
      ...
      if (!heldItemObj) {
        heldItemObj = (pet_items as any)[heldItem];
      }
      lore.push('', `§6Held Item: §${(rarityColors as any)[heldItemObj.tier.toLowerCase()]}${heldItemObj.name}`);

      if (heldItem in (pet_items as any)) {
        lore.push((pet_items as any)[heldItem].description);
      }
      // extra line
      lore.push(' ');
    } else {
    ...
    if (a.active === b.active) {
      if (a.rarity == b.rarity) {
        if (a.type == b.type) {
          ...
          let maxPetA = output
            .filter((x: any) => x.type == a.type && x.rarity == a.rarity)
            .sort((x: any, y: any) => y.level.level - x.level.level);

          maxPetA = maxPetA.length > 0 ? maxPetA[0].level.level : null;

          let maxPetB = output
            .filter((x: any) => x.type == b.type && x.rarity == b.rarity)
            .sort((x: any, y: any) => y.level.level - x.level.level);
...
function getPetLevel(petExp: any, offsetRarity: any, maxLevel: any) {
  const rarityOffset = (PET_RARITY_OFFSET as any)[offsetRarity];
