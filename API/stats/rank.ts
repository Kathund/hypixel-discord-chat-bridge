...
export const getRank = (player: any) => {
  const rank = getPlayerRank(player.rank, player.packageRank, player.newPackageRank, player.monthlyPackageRank);
  const plusColor = `§${(colorToCode as any)[player.rankPlusColor || 'RED']}`;
  const plusPlusColor = `§${(colorToCode as any)[player.monthlyRankColor || 'GOLD']}`;
...
function getPlayerRank(rank: any, packageRank: any, newPackageRank: any, monthlyPackageRank: any) {
  let playerRank =
...
function generateFormattedRank(rank: any, plusColor: any, plusPlusColor: any, prefix: any) {
