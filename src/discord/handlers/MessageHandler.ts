...
export class MessageHandler {
  discord: any;
  command: any;
  constructor(discord: any, command: any) {
  ...
  async onMessage(message: any) {
    try {
      ...
      const images = content.split(' ').filter((line: any) => line.startsWith('http'));
  ...
  stripDiscordContent(message: any) {
    ...
    if (hasMentions) {
      ...
      const replaceUserMention = (match: any, mentionedUserId: any) => {
      ...
      const replaceChannelMention = (match: any, mentionedChannelId: any) => {
  ...
  shouldBroadcastMessage(message: any) {
    const isBot =
